...
As part of the startup configuration generated by the entrypoint script, Kafka Connect uses the [`EnvVarConfigProvider`](https://kafka.apache.org/documentation/#configproviders) to dynamically resolve connector configuration values from environment variables.  
This allows sink connector definitions (e.g., JDBC or MongoDB) to reference environment variables at runtime using the syntax:

```json
"${env:VARIABLE_NAME}"
```

This approach avoids hardcoding database connection parameters directly in connector JSON files, improving **security**, **portability**, and **test isolation**.
