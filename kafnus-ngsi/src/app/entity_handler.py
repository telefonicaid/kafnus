...
async def handle_entity(app, raw_value, datamodel="dm-by-entity-type-database", suffix="", include_timeinstant=True, key_fields=None):
    ...
    service = headers.get("fiware-service", "default")
    servicepath = headers.get("fiware-servicepath", "/")
    entity_type = body.get("entityType", "unknown")
    entity_id = body.get("entityId", "unknown")

    target_table = build_target_table(datamodel, service.lower(), servicepath.lower(), entity_id.lower(), entity_type.lower(), suffix)
